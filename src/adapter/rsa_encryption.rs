...
impl RSAEncrytion for RSA {
    ...
    fn private_key(&self) -> Result<String, &'static str> {
        ...
        match RsaPrivateKey::new(&mut rng, self.bits) {
            Ok(private_key) => match RsaPrivateKey::to_pkcs1_pem(&private_key, LineEnding::CRLF) {
    ...
    fn decrypt(
        ...
        match RsaPrivateKey::from_pkcs1_pem(&private_key.as_str()) {
            Ok(rsa) => match rsa.decrypt(Pkcs1v15Encrypt, &encrypted_message) {
                Ok(messages) => match String::from_utf8(messages) {
